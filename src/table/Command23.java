...
public class Command23 implements Command
{
  ...
  public Command23(long code, DateRepresentation date, long versionAfterExecuting)
  {
    this.code = code;
    this.errorMessage = "";
    this.userName = "";
  ...
  public Command read(MyObjectInputStream in, boolean local) throws IOException
  {
    ...
    {
      if(local)
        return new Command23(in.readLong(), (DateRepresentation) in.readObject(),in.readLong());
      return new Command23(in.readLong(), (DateRepresentation) in.readObject(), Data.version);
  ...
  public void log(IncomingClientThread clientThread, String path, String language)
  {
    try
    {
      ...
      Data.logWriter.println(date.getDay() + "\t" + date.get(date.HOUR_OF_DAY) + ":" + date.get(date.MINUTE) + ":" + date.get(date.SECOND) + "\t" + this.userName + "\t" + this.password + "\tlogoutEmployee\t" + this.errorMessage);
