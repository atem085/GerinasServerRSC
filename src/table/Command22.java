...
public class Command22 implements Command
{
  ...
  public Command22(long code, String password, String language, DateRepresentation date, String version, String javaVersion, long versionAfterExecuting, String bits)
  {
    this.errorMessage = "";
    this.code = code;
  ...
  public Command read(MyObjectInputStream in, boolean local) throws IOException
  {
    ...
    {
      if(local)
        return new Command22(in.readLong(), in.readUTF(), in.readUTF(), (DateRepresentation) in.readObject(), null, null,  in.readLong(), in.readUTF());
      return new Command22(in.readLong(), in.readUTF(), in.readUTF(), (DateRepresentation) in.readObject(), in.readUTF(), in.readUTF(), Data.version, in.readUTF());
  ...
  public void log(IncomingClientThread clientThread, String path, String language)
  {
    try
    {
      ...
      Data.logWriter.println(date.getDay() + "\t" + date.get(date.HOUR_OF_DAY) + ":" + date.get(date.MINUTE) + ":" + date.get(date.SECOND) + "\t" + new String(clientThread.getUser().name) + "\t" + clientThread.getUser().password + "\t" + this.bits + "\tloginCustomerWithData\t" + this.errorMessage);
