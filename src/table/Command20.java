...
public class Command20 implements Command
{
  ...
  public Command20(long code, String password, String language, DateRepresentation date, String version, String javaVersion, long versionAfterExecuting, String bits)
  {
    this.errorMessage = "";
    this.code = code;
  ...
  public Command read(MyObjectInputStream in, boolean local) throws IOException
  {
    ...
    {
      if(local)
        return new Command20(in.readLong(), in.readUTF(), in.readUTF(), (DateRepresentation) in.readObject(), null, null,  in.readLong(), in.readUTF());
      return new Command20(in.readLong(), in.readUTF(), in.readUTF(), (DateRepresentation) in.readObject(), in.readUTF(), in.readUTF(),  Data.version, in.readUTF());
  ...
  public void log(IncomingClientThread clientThread, String path, String language)
  {
    try
    {
      ...
      Data.logWriter.println(date.getDay() + "\t" + date.get(date.HOUR_OF_DAY) + ":" + date.get(date.MINUTE) + ":" + date.get(date.SECOND) + "\t" + new String(clientThread.getUser().name) + "\t" + this.password + "\t" + this.bits + "\tloginCustomerWithUpdate\t" + this.errorMessage);
  ...
  public boolean execute(IncomingClientThread clientThread, DB db, String language)
  {
    ...
    try
    {
      ...
      if(!Consts.version.equals(this.version))
      {
        ...
        System.out.println(te.getValue(0, 1).toString() + "\t" + this.version);
        return false;
      }
