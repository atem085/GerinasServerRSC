...
public class Command29 implements Command
{
  private String errorMessage, language, password, version, javaVersion, bits;
  private long code;
  ...
  public Command29(long code, String password, String language, String version, String javaVersion, DateRepresentation date, long versionAfterExecuting, String bits)
  {
    this.errorMessage = "";
    this.code = code;
  ...
  public Command read(MyObjectInputStream in, boolean local) throws IOException
  {
    ...
    {
      if(local)
        return new Command29(in.readLong(), in.readUTF(), in.readUTF(), null, null,(DateRepresentation) in.readObject(), in.readLong(), in.readUTF());
      return new Command29(in.readLong(), in.readUTF(), in.readUTF(), in.readUTF(), in.readUTF(), (DateRepresentation) in.readObject(), Data.version, in.readUTF());
  ...
  public void log(IncomingClientThread clientThread, String path, String language)
  {
    try
    {
      ...
      Data.logWriter.println(date.getDay() + "\t" + date.get(date.HOUR_OF_DAY) + ":" + date.get(date.MINUTE) + ":" + date.get(date.SECOND) + "\t" +
                             new String(clientThread.getUser().name) + "\t" + clientThread.getUser().password + "\t" + this.bits + "\tloginBuyerWithData\t" +
                             this.errorMessage);
